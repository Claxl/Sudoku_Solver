...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        for val in assignment[unassignedVar]:
            # Create a new partial assignment with the value assigned

            tmp = self.assign_value(assignment, unassignedVar, val)

            # Run AC-3 to remove illegal values from other variables

            if self.inference(tmp, self.get_all_arcs()):
                # Recursively call backtrack on the new partial assignment
        ...
        # If no solution was found, increment the failed backtrack counter

        self.failedBacktrackCount += 1

    def is_complete_assignment(self, assignment):
        """Returns True if the assignment is complete, False otherwise."""

        # Check if each variable has a single value assigned

        return all(len(var) == 1 for var in assignment.values())

    def assign_value(self, assignment, var, val):
        """Returns a new partial assignment with the given value assigned to the given variable."""
    ...
    def inference(self, assignment, queue):
        ...
        while queue:
            ...
            # Revise the assignment for the arc
            if self.revise(assignment, Xi, Xj):
                # If the assignment for Xi is empty, return False
                if len(assignment[Xi]) == 0:
                    return False

                # For each neighboring arc of Xi
                for Xk, _ in self.get_all_neighboring_arcs(Xi):
                    # If Xk is not equal to Xj
                    if Xk != Xj:
    ...
    def getFail(self):
        return self.failedBacktrackCount
    def getCount(self):
...
print("Fail : ", csp.getFail(), "\t","Count : ", csp.getCount())
